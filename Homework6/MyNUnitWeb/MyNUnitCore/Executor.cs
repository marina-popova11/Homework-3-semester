    ...
    private class TestClassRunner
    {
        ...
        public Reporter.TestResult RunSingleTest(TestClassInfo classInfo, MethodInfo method)
        {
            ...
            var testInfo = new Reporter.TestResult
            {
                ...
                AssemblyName = classInfo.ClassType?.Assembly?.GetName().Name ?? "Unknown",
            };

            var ignoreAttribute = method.GetCustomAttribute<IgnoreAttribute>();
            if (ignoreAttribute != null)
            {
                testInfo.Status = Reporter.Status.Ignored;
                testInfo.IgnoreReason = ignoreAttribute.Reason;
                stopwatch.Stop();
                testInfo.DurationMs = stopwatch.Elapsed.TotalMilliseconds;
                return testInfo;
            }
            ...
            stopwatch.Stop();
            testInfo.DurationMs = stopwatch.Elapsed.Milliseconds;
            return testInfo;
        }
